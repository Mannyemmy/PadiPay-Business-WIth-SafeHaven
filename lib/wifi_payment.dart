    ...
    if (payload.type == PayloadType.BYTES) {
      ...
      if (type == 'payment_request') {
        ...
        if (!paymentConfirmation) {
          ...
          showToast('Received request for ₦$amount from $senderName', Colors.green);
          ...
          showDialog<bool>(
            ...
            if (accept == true) {
              ...
              showToast('Accepted ₦$amount from $senderName', Colors.green);
              Nearby().sendBytesPayload(
                  id,
                  utf8.encode(jsonEncode({
                    'type': 'payment_accepted',
              ...
              showToast('Rejected payment from $senderName', Colors.orange);
            }
          });
        }
        ...
        if (widget.deviceType == DeviceType.browser) {
          ...
          if (type == 'payment_accepted') {
            ...
            try {
              ...
              if (!settled) {
                showToast('Transfer failed', Colors.red);
              } else {
            ...
            showToast('Transfer rejected', Colors.red);
          }
        }
      }
    }
...
Future<String> _getUserNameById(String userId) async {
  ...
  Future<bool> requestPermissions() async {
    ...
    Map<Permission, PermissionStatus> requestedStatuses = await notGranted.request();
    List<Permission> denied = [];
    for (var entry in requestedStatuses.entries) {
      if (!entry.value.isGranted) {
        denied.add(entry.key);
      }
    }
    if (denied.isNotEmpty) {
      String deniedNames = denied
        ...
        await Nearby().startAdvertising(
          ...
          onConnectionResult: (String id, Status status) {
            ...
            if (status == Status.CONNECTED && info != null) {
              ...
              showToast('Connected to $id', Colors.red);
            }
          },
          onDisconnected: (String id) {
            setState(() {
  ...
  Future<bool> _settlePaymentToReceiver(Map<String, dynamic> payload, double amount, String purpose) async {
    try {
      ...
      if (ownAccountNumber.isNotEmpty && ownAccountNumber == recipientAccountNumber) {
      ...
      showToast('Error settling payment', Colors.red);
      return false;
    }
  }
}
