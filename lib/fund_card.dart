...
class _FundCardState extends State<FundCard> {
  ...
  double get inputAmount => double.tryParse(_inputAmount.replaceAll(',', '')) ?? 0.0;
  double get displayAmount => isUSD ? inputAmount : inputAmount;
  double get nairaAmount => isUSD ? (inputAmount * _usdToNairaRate) : inputAmount;
  double get fee => isUSD ? (nairaAmount * 0.1) : (inputAmount * 0.05); // 10% for USD (on NGN equiv), 5% for NGN
  ...
  Future<void> _fetchBalance() async {
    try {
      ...
      if (accountNumber != null && accountNumber.length > 4) {
        masked = '*' * (accountNumber.length - 4) + accountNumber.substring(accountNumber.length - 4);
      } else {
  ...
  Future<void> _fetchRate() async {
    ...
    try {
      final fxCallable = FirebaseFunctions.instance.httpsCallable('bridgecardGetFxRate');
  ...
  Future<bool> _refundToUser(Map<String, dynamic> userDetails, Map<String, dynamic> companyVa, double totalAmount) async {
    try {
  ...
  void _updateAmounts() {
    setState(() {
      _inputAmount = _amountController.text.isEmpty ? "0.00" : _amountController.text;
    });
  }
  ...
  Future<void> _performFunding() async {
    if (inputAmount <= 0) {
      showToast('Enter a valid amount', Colors.red);
      return;
    }
    ...
    try {
      ...
      final errorMsg = e.toString().contains('Account name is required')
          ...
          : 'Funding failed';
      if (refunded) {
        showToast('$errorMsg, but amount refunded', Colors.orange);
      } else {
        showToast(
          transferSuccess
                              ...
                              children: [
                                Column(
                                  ...
                                  children: [
                                    ...
                                    SizedBox(height: 5),
                                    _isAccountLoading
                                        ? _shimmerPlaceholder(width: 80, height: 20)
                                        : Text(
