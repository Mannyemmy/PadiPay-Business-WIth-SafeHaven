...
import 'package:padi_pay_business/cashback/cashback_service.dart';
...
class _PayBillsPageState extends State<PayBillsPage> {
  ...
  Future<Map<String, double>> _settleCashbackAfterSuccess({
    ...
    var settledCashbackUsed = cashbackUsed;
      ...
      try {
        ...
        try {
          await _rollbackCashbackIfNeeded(
            ...
            accountId: accountId,
            narration: 'Cashback rollback for $sourceType purchase',
          );
          settledCashbackUsed = 0;
        } catch (rollbackError) {
    ...
    return {'used': settledCashbackUsed, 'earned': cashbackEarned};
  }

  Widget _buildCashbackToggleBox() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.savings_outlined, color: primaryColor, size: 18),
                  ...
                  if (id != null) {
                    setState(() {
                      ...
                      final selectedSub = cableSubscriptions.firstWhere(
                        ...
                        orElse: () => cableSubscriptions.first,
                      );
                      cableAmountController.text =
  ...
  Widget _iconForId(String id) {
    final netLogo = _getNetworkLogo(id);
    if (netLogo.isNotEmpty) {
      return Image.network(netLogo, width: 24, height: 24);
    }
    final cableLogo = _getCableLogo(id);
    if (cableLogo.isNotEmpty) {
      return Image.network(cableLogo, width: 24, height: 24);
    }
    final lower = id.toLowerCase();
    if (lower.contains('mtn')) {
      return SvgPicture.asset(
