...
class _BusinessInformationState extends State<BusinessInformation> {
  ...
  final Map<String, List<String>> stateToCities = {
    ...
    'Edo': ['Esan North-East', 'Esan Central', 'Esan West', 'Egor', 'Ikpoba', 'Central', 'Etsako Central', 'Igueben', 'Oredo', 'Ovia SouthWest', 'Ovia South-East', 'Orhionwon', 'Uhunmwonde', 'Etsako East', 'Esan South-East'],
    ...
    'Kogi': ['Adavi', 'Ajaokuta', 'Ankpa', 'Bassa', 'Dekina', 'Ibaji', 'Idah', 'Igalamela-Odolu', 'Ijumu', 'Kabba/Bunu', 'Lokoja', 'Mopa-Muro', 'Ofu', 'Ogori/Mangongo', 'Okehi', 'Okene', 'Olamabolo', 'Omala', 'Yagba East', 'Yagba West'],
    ...
    'Niger': ['Agaie', 'Agwara', 'Bida', 'Borgu', 'Bosso', 'Chanchaga', 'Edati', 'Gbako', 'Gurara', 'Katcha', 'Kontagora', 'Lapai', 'Lavun', 'Magama', 'Mariga', 'Mashegu', 'Mokwa', 'Muya', 'Pailoro', 'Rafi', 'Rijau', 'Shiroro', 'Suleja', 'Tafa', 'Wuse'],
    ...
    'Ondo': ['Akoko North East', 'Akoko North West', 'Akoko South Akure East', 'Akoko South West', 'Akure North', 'Akure South', 'Ese-Odo', 'Idanre', 'Ifedore', 'Ilaje', 'Ile-Oluji', 'Okeigbo', 'Odigbo', 'Okitipupa', 'Ondo East', 'Ondo West', 'Ose', 'Owo'],
    'Osun': ['Aiyedaade', 'Aiyedire', 'Atakumosa East', 'Atakumosa West', 'Boluwaduro', 'Boripe', 'Ede North', 'Ede South', 'Egbedore', 'Ejigbo', 'Ifedayo', 'Ifelodun', 'Ilesha East', 'Ilesha West', 'Ila', 'Ife Central', 'Ife East', 'Ife North', 'Ife South', 'Irepodun', 'Irewole', 'Isokan', 'Iwo', 'Obokun', 'Odo-Otin', 'Ola-Oluwa', 'Olorunda', 'Oriade', 'Orolu', 'Osogbo'],
    ...
    'Taraba': ['Ardo-kola', 'Bali', 'Donga', 'Gashaka', 'Gassol', 'Ibi', 'Jalingo', 'KarimLamido', 'Kurmi', 'Lau', 'Sardauna', 'Takum', 'Ussa', 'Wukari', 'Yorro', 'Zing'],
  ...
  Future<void> _loadData() async {
    ...
    if (fsDoc.exists) {
      ...
      if (map != null) {
        ...
        // Prefer BVN from user doc's qoreIdData.verification.metadata.idNumber if present (source of truth)
        try {
          final userDoc = await FirebaseFirestore.instance.collection('users').doc(user.uid).get();
          final userMap = userDoc.exists ? (userDoc.data() ?? <String, dynamic>{}) : <String, dynamic>{};
          ...
          if (idNumber != null && idNumber.isNotEmpty) {
            _bvnController.text = idNumber;
          } else {
            _bvnController.text = '';
          }
        } catch (e) {
          // Fallback to business_data BVN if any error occurs while reading user doc
          _bvnController.text = '';
        }
        ...
        _descController.text = map['desc'] ?? '';
        selectedRegState = map['regState'];
        if (selectedRegState != null) {
          updateRegCities(selectedRegState!);
        }
  ...
  Future<void> _saveData() async {
    ...
    if (_bvnController.text.length != 11) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('BVN must be exactly 11 digits')));
      return;
    }
    setState(() {
      isLoading=true;
    ...
    await FirebaseFirestore.instance
        .collection('businesses')
        .doc(user.uid)
        .set({'business_data': map, 'business_fixed': true}, SetOptions(merge: true));
    setState(() {
      isLoading=false;
    });
  ...
  String _format(String value) {
    ...
    formatted = formatted.replaceAllMapped(RegExp(r'(?<!^)([A-Z])'), (m) => ' ${m.group(0)}');
        ...
        return StatefulBuilder(
          builder: (context, setBottomSheetState) {
            final filteredItems = items.where((item) {
              return _format(item).toLowerCase().contains(searchQuery.toLowerCase());
            }).toList();
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          title: Text(_format(item),style: GoogleFonts.inter(fontSize: 14),),
                          onTap: () {
                            onSelected(item);
