...
class CashbackService {
  ...
  static double calculateCashback(double baseAmountNaira) {
    if (baseAmountNaira <= 0) return 0;
    return double.parse((baseAmountNaira * cashbackRate).toStringAsFixed(2));
  }
  ...
  static Future<double> getCashbackBalance(String uid) async {
    ...
    return (doc.data()?['cashback']?['balance'] as num?)?.toDouble() ?? 0;
  }
  ...
  static Future<double> recordCashbackEarned({
    ...
    await _firestore.runTransaction((tx) async {
      ...
      final currentBalance =
          (userSnap.data()?['cashback']?['balance'] as num?)?.toDouble() ?? 0;
      final newBalance = double.parse(
        (currentBalance + cashbackAmount).toStringAsFixed(2),
      );
