...
class _ConfirmOtpBottomSheetState extends State<ConfirmOtpBottomSheet> {
  ...
  Future<void> _verifyOtp() async {
    ...
    if (smsCode.length != 6) {
      showToast('Please enter full 6-digit code', Colors.red);
      return;
    }
    ...
    try {
      ...
      if (e.code == 'provider-already-linked') {
        ...
        showToast('Invalid OTP: ${e.message}', Colors.red);
      }
    } catch (e) {
      showToast('Verification failed: $e', Colors.red);
    } finally {
      if (mounted) {
        setState(() => _isLoading = false);
      }
    ...
    await FirebaseAuth.instance.verifyPhoneNumber(
      ...
      verificationCompleted: (PhoneAuthCredential credential) async {
        try {
          ...
          setState(() => _isLoading = false);
          showToast('Auto verification failed', Colors.red);
        }
      },
      verificationFailed: (FirebaseAuthException e) {
        setState(() => _isLoading = false);
        showToast(e.message ?? 'Resend failed', Colors.red);
      },
      codeSent: (String verificationId, int? resendToken) {
        ...
        showToast('Code resent', Colors.green);
        // Clear OTP fields
